...
bool ComandoCliente :: comandoNewRecibirRespuesta(SocketConnect &socket,
 ...
 //falta verificar que coicidan e imprmir, 
 //se deberia hacer funcion aparte (y usarla para revoke)
...
//falta una funcion que envie los datos y tenga un try except
bool ComandoCliente :: comandoNew(SocketConnect &socket ,
    ...
    if (protocolo.enviarString(nombre,socket) == -1) return false; 
//falta funcion string que envie de a pedazos 
//y otra que reciba de a pedazos dentro de protocolo
    ...
    uint8_t rta;
    if (this->comandoNewRecibirRespuesta(socket,protocolo,
...
bool ComandoCliente :: comandoRevoke(SocketConnect &socket,
    ...
    //Esto se deberia hacer en una funcion aparte
    uint32_t hash = certificado.calcularHash(); 
    std::cout << "Hash calculado: " << hash << std::endl;
    ...
    uint8_t rta = this->respuestaDelServidor(socket);
    if (rta == 1) {
